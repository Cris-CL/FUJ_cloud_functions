...
--      WHEN pay.payout_status like '%paid' THEN om.shipping_shop_amount
      WHEN om.cancelled_at is not null and om.cancelled_at > sub_n.payout_date then om.shipping_shop_amount
      WHEN om.cancelled_at is not null and om.cancelled_at < sub_n.payout_date and pay.type = 'refund' then 0
...
from totales
where amount > 0
order by CAST(control_number as int64) desc, account
