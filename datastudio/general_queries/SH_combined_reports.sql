...
--- From here the condensed report is created, the transaction date
--- will be taken as the first day of the month that it happened
---
select

balance,
control_number,
DATE_TRUNC(accrual_date, month) as transaction_date,
settlement_date as accrual_date,
suppliers,
account,
tax_distinction,
sum(amount) as total,
tax_calculation_distinction,
sum(tax_total) as tax_tot,
remarks,
item,
department,
memo_tag,
date_3 as settlement_date,
settlement_account,
sum(settlement_amount) as settlement_total_amount,
sum(real_amount) as real_total,
order_number
...
order by control_number desc, order_number
